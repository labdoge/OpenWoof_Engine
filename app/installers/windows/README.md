...
這裡保留既有 Windows installer 作為 repo 內部歸檔與比對用途。依照目前 release 規則，公開發布資產應使用 `app/portable/windows/WoofyChatty_0.1.0_x64_portable.zip`，不要把 installer 當作主要 release asset。
...
This folder keeps the existing Windows installers for repository archival and comparison. Under the current release rule, public release assets should use `app/portable/windows/WoofyChatty_0.1.0_x64_portable.zip`, not the installers.
